...
public class ProductManager {
	...
	public List<Product> getActiveProductsSortedByPrice() {
		...
		// metodu yazın
		List<Product> productActive = products.values().stream()
	...
	public double calculateAveragePriceInCategory(String category) {
		...
		double sum = products.values().stream().filter(w -> w.getCategory().equals(category))
				.mapToDouble(w -> w.getPrice()).sum();
		int count = (int) products.values().stream().filter(w -> w.getCategory().equals(category)).count();

		OptionalDouble countOp = OptionalDouble.of(count);

		if (countOp.equals(OptionalDouble.of(0)) || countOp.equals(null))
			return 0.0;
		else
			return sum / count;
	}
	...
	public Map<String, Double> getCategoryPriceSum() {
		...
		Map<String, Double> groupProducts = products.values().stream()
